...
internal class StandardKtTest {
  ...
  inner class Counter {
    ...
    @DisplayName("arrays")
    fun arrays() {
      assertThat(counter(emptyArray<String>())).isEqualTo(emptyMap<String, Int>())
      assertThat(counter(arrayOf(1, 2, 1))).isEqualTo(mapOf(1 to 2, 2 to 1))
      assertThat(counter(arrayOf("a", "d", "d", "d"))).isEqualTo(mapOf("a" to 1, "d" to 3))

      assertThat(counter(byteArrayOf(1, 2, 1)))
        .isEqualTo(mutableMapOf(1.toByte() to 2, 2.toByte() to 1))
      assertThat(counter(charArrayOf('a', 'b', 'a'))).isEqualTo(mapOf('a' to 2, 'b' to 1))
      assertThat(counter(shortArrayOf(1, 2, 1)))
        .isEqualTo(mapOf(1.toShort() to 2, 2.toShort() to 1))
      assertThat(counter(intArrayOf(1, 2, 1))).isEqualTo(mapOf(1 to 2, 2 to 1))
      assertThat(counter(uintArrayOf(1u, 2u, 1u))).isEqualTo(mapOf(1u to 2, 2u to 1))
      assertThat(counter(longArrayOf(1L, 2L, 1L))).isEqualTo(mapOf(1L to 2, 2L to 1))
      assertThat(counter(floatArrayOf(1F, 2F, 1F))).isEqualTo(mapOf(1F to 2, 2F to 1))
      assertThat(counter(doubleArrayOf(1.0, 2.0, 1.0))).isEqualTo(mapOf(1.0 to 2, 2.0 to 1))
